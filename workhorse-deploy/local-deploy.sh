...
echo ""
echo "--- Tables ---"
docker exec workhorse-postgres psql -U postgres -d workhorse -c '\dt' \
  || fail "Listing tables"

echo ""
echo "--- Projects ---"
docker exec workhorse-postgres psql -U postgres -d workhorse -c 'SELECT slug, priority FROM projects ORDER BY priority, slug;' \
